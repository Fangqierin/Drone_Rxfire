...
class UAV:
        ...
        if (time.time() - self.tel_timestamp) > self.telemetry_freq:
            ...
            if len(msg) > 0:
                ...
                if self.home_location is not None and self.current_location is not None:
                    ...
                    #file_w.write(str(x) + '\t' + str(y) + '\t' +'n') # + str(battery[battery.find("level=")+6:]) + '\n')
                    file_w.close()
                    print(str(x) + '\t' + str(y) + '\t' + str(battery[battery.find("level=")+6:]))
                    self.zmq_tel_socket.send(str("@@@U_" + self.uav_id + "***" + "DISTANCE***" + str(x) + "***" + str(y) + "***").encode())
                self.tel_timestamp = time.time()
    ...
    def create_zmq(self, zmq_type, con_string, prefix="", verbose=False):
        ...
        if "PUB" in zmq_type:
            ...
            sock_new.setsockopt(zmq.SUBSCRIBE, b'')
            if verbose:
    ...
    def get_data(self, socket, verbose):
        while True:
            data = socket.recv()
            data=data.decode()
            #print("here I want to see the data: !!!!!!!!!!!!", data)
    ...
    def goto(self, dNorth, dEast, gotoFunction):
        ...
        while self.vehicle.mode.name == "GUIDED":  # Stop action if we are no longer in guided mode.
            ...
            if remainingDistance <= targetDistance * 0.01:  # Just below target, in case of undershoot.
                print(remainingDistance,"Reached target!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                break;
            time.sleep(2)
    ...
    * send_global_velocity - Sets velocity components using SET_POSITION_TARGET_GLOBAL_INT command
    ...
    def go_to(self, vehicle, location, verbose):
        ...
        print("fangqi look here!!!!!!!!!!!!!", point,str(location))
        vehicle.simple_goto(point)
        return True

    def go_seq(self, vehicle, location, verbose):
        if verbose:
            print(self.prefix + " Beginning mission")
        #create cmds list
        cmds = vehicle.commands
        #clear any existing commands
        cmds.clear()
        #get array from file
        loc_arr = self.read_seq()
        for r in loc_arr:
            cmds.add(Command( 0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT, 0, 0, 0, 0, 0, 0, float(r[0]), float(r[1]), float(r[2])))
        #upload cmds
        cmds.upload()
        ...
        while True:
            ...
            if nextwaypoint==4: #Dummy waypoint for exit
