...
import zmq
import FQ_gcs_pubsub as gcs
#import uav_pubsub34 as uav
import FQ_drone_mobility as uav
...
if __name__ == "__main__":
    ...
    gcs_obj = None
    time.sleep(0.1)
    ################################## Start the network simulator NS3 
    ...
    for i in range(args.instance):
        p_sitl = subprocess.Popen("xterm -T SITL_" + str(i) +
                                  " -e dronekit-sitl copter --instance --speedup=1 " + str(i), shell=True)
        # p_sitl = subprocess.Popen("xterm -T SITL_" + str(i) +
    ...
    if args.verbose == 1 or args.verbose == 3:
        ...
        ver = False    # if args.verbose == 1 or args.verbose == 3:
    # else:
    #     ver = False

    #gcs.main(5501, 5500, args.instance, ver)  # NS3
    #gcs.main(5551, 5550, args.instance,False)  # DIRECT
    gcs.main(5551, 5550, args.instance,False)  # DIRECT
'''
