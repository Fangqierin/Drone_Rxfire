...
class MyGCS(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def send_data(self, message, uav_id, sock, verbose):
        try:
            ...
            #print(self.textedit_control,msg_send)
            #print(sock)
           # sock.send(b"aaa")    ############################# HERE how to set the msg_send to b'' 
           # print(msg_send)
            sock.send(str(msg_send).encode())
            #print(socket.setsockopt_string("msg_send"))
            #sock.send(sock.setsockopt_string(msg_send))
            #sock.send(msg_s)
        except:
