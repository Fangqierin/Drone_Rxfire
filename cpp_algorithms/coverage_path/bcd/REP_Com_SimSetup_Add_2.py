    ...
    for wind in [5,15]:#:
        for time in [60]:# [1,20, 40,60]:
            for seed in range(10):#[0]:# [0,1,2]:# 
                ...
                foldername=f"REP_FQ_Tmp_U2_{wind}_{time}_{seed}"
                #CreateDyRxfire(Bardata,fir_name,dir, [2],wind=wind)
                CreateRandomRxfire(Bardata,fir_name,dir, [1],wind=wind,sed=seed,Inputdict=Inputdict,Offset=50)
                # fir_name=f"REP_FQ_Rand_{wind}_{seed}"
                # #foldername=f"REP_FQ_Tmp_{wind}_{time}_{seed}"
                # #CreateDyRxfire(Bardata,fir_name,dir, [2],wind=wind)
                # CreateRandomRxfire(Bardata,fir_name,dir, [2],wind=wind,sed=seed,Inputdict=Inputdict,Offset=80)
