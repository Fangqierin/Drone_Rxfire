...
if __name__ == "__main__":
    ...
    Missions['f']=[ 5, 1]  #Other area 
    Missions['a']=[ 10, 3]  #track the fire 
    ...
    Res=10
    EFA,EFAdict,Primdict =GetEFFNext(init,end,Bursite,dir,foldername, Res=Res)
    Task_mission=GenTasks(init,end,Bursite,dir,foldername,Missions, Res=Res)
    #GetEFA("EFAdict.csv") 
    ########################## Do decomposition~ 
    DecomposeSize=200
    DecomposeSize=int((DecomposeSize/Res)*Res)
    Area,Fire=Decomposition(EFA, DecomposeSize,Res,Task_mission) # Need put tasks there!!!
        ########################## define drone as 
    DroneNum=3
    Bidders=[]
    Drones=[]
    ...
    for i in range(DroneNum):
        Drones.append(Drone(id=i,sensortype=sensortype[i], FoV=FoV[i],speed=speeds[i]))
    
    Drones,Bidders=Auction(Area, Fire, Drones,EFA, Res,Missions)
