...
if __name__ == "__main__":
    ...
    logfile=f"./Results/REP_logFQ_{Unit}_{DroneNum}_{STtime}"
    log=open(logfile, "w")
    Simfile=f"./Results/REP_SimpleFQ_{Unit}_{DroneNum}_{STtime}"
    ...
    for seed in range(10):
        ...
        ########################################################
        if Unit==1:
            fir_name=f"REP_FQ_Rand_{wind}_{seed}"
            foldername=f"REP_FQ_Tmp_{DroneNum}_{STtime}_{seed}"
            Bursite=(702460.0,4309700.0,702860.0,4310200.0 )
        if Unit==2:
            fir_name=f"REP_FQ_Rand_U2_{wind}_{seed}"
            foldername=f"REP_FQ_Tmp_U2_{DroneNum}_{STtime}_{seed}"
            Bursite=(702850.0,4310030.0,703250.0,4310360.0)
        if Unit==3:
            fir_name=f"REP_FQ_Rand_U3_{wind}_{seed}"
            foldername=f"REP_FQ_Tmp_U3_{DroneNum}_{STtime}_{seed}"
