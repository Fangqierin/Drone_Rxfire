...
class TaskGenerator(KnowledgeEngine):
    ...
    def Remove_Dup(self,f,id):
        self.retract(f)
        print(f"Ignore detected fire at {id}")
    ...
    def ShiftUKtoN(self,f,f1,id,t):
        ...
        print(f"-->Gird {id}: UK -> N at {t}")
        tasks[id].pop('FD', None)
        print(f"-->Delete FD at grid {id}")
